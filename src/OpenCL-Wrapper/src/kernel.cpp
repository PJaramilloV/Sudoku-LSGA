...
kernel void crossoverKernel(
  ...
  if(member < N){
    ...
    const uint gridPostition = row*9 + column;
    if(do_swap){
...
kernel void mistakesAddKernel(global uchar* mistakes, global uchar* scores, const uint N){
  ...
  shared_array[thLocal] = 0;

  barrier(CLK_LOCAL_MEM_FENCE);

  if (member < N){
    shared_mem[thLocal-1] = input[member*18 + thLocal];
    barrier(CLK_LOCAL_MEM_FENCE);
    if (thLocal| == 0){
      uint errors = 0;
      for (uint i=0; i<18; i++){
...
kernel void mutateRowKernel(
  ...
  if (member < N){
    ...
    for(uint i=0; i < 9; i++){  // choose columns to swap numbers from
      ...
      second = (second_selected & second) | ((first_selected & !second_seleted) & val) | ((!first_selected) & MAX_UINT );
...
kernel void lsga_col_kernel(global uchar* grid, global uchar* occupancy, global uchar* mistakes, const uint N){
  ...
  if(member < N){
    for (uint col=0; col < 9; col++){
      ...
      uint other = somehow get another; // TODO: this line
